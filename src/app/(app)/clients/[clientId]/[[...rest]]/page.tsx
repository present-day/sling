...
export default function ClientWorkspacePage() {
	...
	const toolsQuery = trpc.qbo.listTools.useQuery(
		{ clientId },
		{ enabled: segment === "tools" },
	);
			...
			<div className="space-y-4">
				...
				<Card>
					...
					<CardContent className="text-muted-foreground text-sm">
						Use the sidebar for entities, reports, the tool explorer, and chat.
						Each entry maps to QuickBooks MCP tools (see PLAN.md).
					</CardContent>
				</Card>
			</div>
		);
	...
	if (segment === "tools") {
		return (
			<div className="space-y-4">
				<h1 className="font-heading text-2xl font-semibold">Tool Explorer</h1>
				<p className="text-muted-foreground text-sm">
					All tools from <span className="font-mono text-xs">list_tools</span>{" "}
					for this client.
				</p>
				{toolsQuery.isLoading ? (
					<p className="text-muted-foreground text-sm">Loading tools…</p>
				) : toolsQuery.error ? (
					<p className="text-destructive text-sm">{toolsQuery.error.message}</p>
				) : (
					<ul className="max-h-[560px] list-inside list-disc overflow-auto text-sm">
	...
	if (segment === "chat") {
		return (
			<div className="space-y-2">
				...
				<p className="text-muted-foreground text-sm">
					Streaming chat with scoped MCP tools ships via{" "}
	...
	return (
		<div className="space-y-2">
			...
			<p className="text-muted-foreground text-sm">
				Connect QuickBooks MCP or use the Tool Explorer to run tools against
				this client.
			</p>
		</div>
	);
