...
This app lives in the Bun monorepo: run **`bun install` and `bun run dev` from the repository root** (see root `README.md`). You can also work inside `apps/console` once the workspace is installed.
...
See `docs/architecture.md` and `PLAN.md` for the full product spec. This repo implements the stack (Next.js 16, tRPC, Better Auth, Drizzle/SQLite, direct QBO HTTP) with routes and stubs where noted in code comments.
